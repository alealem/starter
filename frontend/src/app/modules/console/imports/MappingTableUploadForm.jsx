...
export default function MappingTableUploadForm() {
    ...
    const onSubmit = React.useCallback(
        form => {
            ...
            apiClient
                .doPost('/imports/mapping-table', { headers, body: formData })
                .then(() => {
                    notification.notify('Mapping Table File is Uploaded', { type: 'success' });
                    form.setSubmitting(false);
                })
                .catch(() => {
                    notification.notify('Mapping Table File could not be Uploaded', {
                        type: 'warning'
                    });
                    form.setSubmitting(false);
                });
        ...
        <Formix.Form fieldsInitializer={defineFields} onSubmit={onSubmit} initialValue={null}>
            {({ fields, handleSubmit, isValid, isSubmitting }) => (
                <form noValidate onSubmit={handleSubmit}>
                    <Layout.Card
                        header="Datei mit Brücken-Tabelle zum hochladen auswählen"
                        footer={
